...
def manage_meal(meal_id):
    ...
    if not session.get('loggedin') or 'user_id' not in session:
        flash('You must be logged in to manage your meals.', 'error')
        return redirect(url_for('login_bp.login'))
    ...
    if action == 'delete':
        # 2. Call the database function to delete the meal entry
        # The delete_meal_entry function should also check if the meal belongs to this user_id
        success = delete_meal_entry(meal_id, user_id)

        if success:
            flash("Meal deleted successfully!", 'success')
        else:
            flash("Meal not found or could not be deleted.", 'error')
    
    # 3. Redirect back to the profile page to show the updated history
    return redirect(url_for('login_bp.profile'))
